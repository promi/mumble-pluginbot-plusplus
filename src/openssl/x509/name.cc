...
namespace OpenSSL::X509
{
  Name::Name ()
  {
    ...
    if (m_name == nullptr)
      {
        throw std::string ("X509_NAME_new () failed");
      }
  }
  ...
  void Name::add_entry (const std::string &field, const std::string &text)
  {
    if (field == "C" && text.size () > 2)
      {
        throw std::string ("Country name too long (should be 2 characters long)");
      }
    if (X509_NAME_add_entry_by_txt (m_name, field.c_str (), MBSTRING_UTF8,
                                    reinterpret_cast<const unsigned char*> (text.c_str ()),
                                    text.size (), -1, 0) != 1)
      {
        throw std::string ("X509_NAME_add_entry_by_txt () failed");
      }
  }
}
